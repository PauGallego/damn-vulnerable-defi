...
contract CurvyPuppetChallenge is Test {
    ...
    /**
      Explaination:
        The attack takes advantage of the CurvyPuppetLending contract's reliance on the Curve pool for price oracle data.
        By manipulating the Curve pool's liquidity through flash loans from Aave V2 and V3, the attacker can skew the price of stETH relative to ETH.
        This manipulation allows the attacker to reduce the perceived value of the collateral (DVT) held by the users in the lending contract.
        Once the collateral value is sufficiently lowered, the attacker can liquidate the users' positions, seizing their DVT collateral.
    */
    ...
    function test_curvyPuppet() public checkSolvedByPlayer {
        ...
        //Transfer LP and WETH to attacker contract
...
contract Attacker {
    ...
    constructor(address _v2Pool, address _v3Pool, IERC20 _lp_token, IStableSwap _curvePool, CurvyPuppetLending _lending, WETH _weth, IERC20 _stETH, address[3] memory _users, address _permit2, address _dvt, address _treasury) payable {
        ...
        IPermit2(permit2).approve({token:address(lp_token), spender:address(lending),amount:3e18,expiration:uint48(block.timestamp)});
    }
    ...
    // second call : flash loan from V3 pool
    ...
    // first call: flash loan from V2 pool
    ...
    //On receive ether liquidate users
